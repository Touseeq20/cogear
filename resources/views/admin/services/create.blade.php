...
<div class="max-w-4xl px-4">
    <div class="mb-8 flex items-center">
        ...
        <h3 class="text-2xl font-bold text-slate-800">Create New Service</h3>
    </div>
    ...
    <div class="bg-white rounded-[2rem] shadow-sm border border-slate-100 p-10 px-4">
        <form action="{{ route('admin.services.store') }}" method="POST" class="space-y-8">
            ...
            <div class="px-4">
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-100 focus:border-blue-600 transition">
                @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
            </div>

            <div class="px-4">
                <label class="block text-sm font-bold text-slate-700 mb-2">Icon Name (Optional)</label>
                <input type="text" name="icon" value="{{ old('icon') }}" placeholder="brain, robot, code, etc." class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-4 focus:ring-blue-100 focus:border-blue-600 transition">
